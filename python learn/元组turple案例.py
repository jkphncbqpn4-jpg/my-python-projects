...
print(f"语文最高分{max(chinese_scores)}, 语文最低分{min(chinese_scores)}, 语文平均分{sum(chinese_scores)/len(chinese_scores):.1f}")
print(f"数学最高分{max(math_scores)}, 数学最低分{min(math_scores)}, 数学平均分{sum(math_scores)/len(math_scores):.1f}")
print(f"英语最高分{max(english_scores)}, 英语最低分{min(english_scores)}, 英语平均分{sum(english_scores)/len(english_scores):.1f}")

print()

#查找成绩优秀（平均分大于90）的学生，并输出
excellent = [i[1] for i in students if (i[2]+i[3]+i[4])/3>90]
print(f"成绩优秀的学生：{excellent}")
